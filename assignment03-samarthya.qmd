...
lower_pattern = "(?i)High school|Some college|Certificate|Associate|GED|No education listed"
bachelor_pattern = "(?i)Bachelor"
higher_pattern = "(?i)Master|Professional"
phd_pattern = "(?i)Doctoral|Doctorate|PhD"
...
remote_pd['experience_jitter'] = remote_pd['experience_years'] + np.random.normal(0, 0.18, len(remote_pd))
...
for i, remote_type in enumerate(['Remote', 'Hybrid', 'Onsite']):
    ...
    if len(data) > 0:
        ...
        # Scatter plot with jitter (using go.Scatter for consistency). Include occupation in hover.
        fig4.add_trace(
            go.Scatter(
                ...
                mode='markers',
                name=f"{remote_type}",
                marker=dict(color=colors[remote_type], size=5, opacity=0.7, line=dict(width=0.15, color='DarkSlateGrey')),
                hovertemplate=f'{remote_type}<br>Occupation: %{{customdata[0]}}<br>Experience: %{{x:.1f}} years<br>Salary: $%{{y:,.0f}}<extra></extra>',
                customdata=np.stack([data['LOT_V6_SPECIALIZED_OCCUPATION_NAME']], axis=-1),
                showlegend=False
            ),
            row=1, col=i+1
        )
