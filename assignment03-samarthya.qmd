...
<!-- Updated to reflect LOT_OCCUPATION_NAME analysis with improved imputation methodology -->
...
from pyspark.sql.functions import col, count, median, when, isnan, isnull, countDistinct
...
# Ensure kaleido is available for static image generation (modern Plotly standard)
import kaleido
...
# Helper function to display figures based on output format
def get_output_format():
    """Get the current Quarto output format"""
    return os.environ.get('QUARTO_OUTPUT_FORMAT', 'html')

def display_figure(fig, filename_base):
    """
    Enhanced display function for multi-format compatibility using modern Plotly standards
    - HTML: Interactive Plotly figure  
    - PDF/DOCX: Static PNG images with high-quality kaleido rendering
    - Comprehensive error handling and directory management
    """
    output_format = get_output_format()
    import sys
    sys.stderr.write(f"[PDF_LOG] Processing figure {output_format}\n")
    sys.stderr.flush()
    print(f"SAMARTHYA::::::Current output format: {output_format}")
    
    # Always generate static PNG for PDF/DOCX compatibility
    static_filename = f"images/{filename_base}.png"
    html_filename = f"images/{filename_base}.html"

    # Use modern kaleido engine (default, no deprecated engine parameter)
    ...
    if os.path.exists(static_filename):
        file_size = os.path.getsize(static_filename)
        print(f"  File size: {file_size:,} bytes")
        if file_size < 1000:
            print(f"  Warning: File size seems small, image may not have rendered properly")
        else:
            print(f"  Error: Static file was not created")
    
    if "html" in output_format:
        
        fig.show(renderer="notebook", config={
            'displayModeBar': True,
            'displaylogo': False,
        ...
        fig.write_html(html_filename, config={'displayModeBar': True})
        print(f"Interactive HTML saved: {html_filename}")
            
    else:
        sys.stderr.write(f"[PDF_LOG] Processing not HTML figure: {output_format}:{static_filename}\n")
        sys.stderr.flush()
        # For PDF/DOCX formats, provide Quarto-compatible image reference
        if os.path.exists(static_filename):
...
for salary_col in salary_columns_to_impute:
    if salary_col in df_clean.columns:
        try:
            # First, clean extreme outliers and unrealistic values
            print(f"   • Cleaning extreme outliers and unrealistic values for {salary_col}")
            df_clean = df_clean.withColumn(salary_col,
                when((col(salary_col) < 15000) | (col(salary_col) > 500000),  # Remove unrealistic salaries
                     None)
                .otherwise(col(salary_col))
            )
            
            # Calculate median by LOT_OCCUPATION_NAME + EMPLOYMENT_TYPE_NAME for more accurate imputation
            ...
            # Convert to dict for lookup with composite key
            occupation_employment_dict = {
                (row["LOT_OCCUPATION_NAME"], row["EMPLOYMENT_TYPE_NAME"]): row["group_median"] 
                for row in occupation_employment_medians.collect()
            }
            
            # Calculate fallback medians by employment type only
            ...
            employment_dict = {
                row["EMPLOYMENT_TYPE_NAME"]: row["group_median"] 
                for row in employment_medians.collect()
            }

            # Get overall median as final fallback
            ...
            print(f"   • Occupation+Employment specific medians: {len(occupation_employment_dict)} combinations")
            print(f"   • Employment type fallback medians: {len(employment_dict)} types")

            # Function to get most appropriate median (hierarchical fallback)
            def get_occupation_employment_median(occupation, employment_type):
                # Try occupation + employment type first
                composite_key = (occupation, employment_type)
                if composite_key in occupation_employment_dict:
                    return occupation_employment_dict[composite_key]
                # Fallback to employment type only
                elif employment_type in employment_dict:
                    return employment_dict[employment_type]
            ...
            get_median_udf = udf(get_occupation_employment_median, FloatType())

            # Apply improved imputation
            df_clean = df_clean.withColumn(salary_col,
                when((col(salary_col).isNull()) | (col(salary_col) <= 0),
                     get_median_udf(col("LOT_OCCUPATION_NAME"), col("EMPLOYMENT_TYPE_NAME")))
                .otherwise(col(salary_col))
            )

            # Count how many values were imputed
            imputed_count = df_clean.filter((col(salary_col).isNull()) | (col(salary_col) <= 0)).count()
            print(f"   • Imputed {imputed_count:,} missing/invalid {salary_col} values with occupation-specific medians")
...
print("Interactive HTML saved: 03_education_level_analysis.html")
print("Static images automatically generated by Plotly PNG renderer")
